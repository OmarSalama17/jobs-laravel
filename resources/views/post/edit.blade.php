<x-layout>
        <section class="px-4 md:px-16 lg:px-24 xl:px-32 w-full">
            <h1 class="text-[22px] font-bold">edit : {{ $data->title }}</h1>
        <form method="POST" action="/post/{{$data->id}}" class="grid sm:grid-cols-2 gap-3 sm:gap-5 max-w-2xl mx-auto text-slate-300 mt-16 w-full">
            @method('PUT')
            @csrf
            <div>
                ...
                <div class="flex items-center pl-3 rounded-lg overflow-hidden border border-slate-700 focus-within:border-pink-500">
                    ...
                    <input value="{{ old('title' , $data->title) }}" placeholder="Enter your name" class="w-full p-3 bg-transparent outline-none" type="text" name="title" />
                </div>
                @error('title')
                {{ $message }}
                @enderror
            ...
            <div>
                ...
                <div class="flex items-center pl-3 rounded-lg overflow-hidden border border-slate-700 focus-within:border-pink-500">
                    ...
                    <input value="{{ old('author' , $data->author) }}" placeholder="Enter your author" class="w-full p-3 bg-transparent outline-none" type="text" name="author" />
                </div>
            </div>
            <div class="sm:col-span-2">
                <p class="mb-2 font-medium">Message</p>
                <textarea name="body" rows="8" placeholder="Enter your message" class="focus:border-pink-500 resize-none w-full p-3 bg-transparent outline-none rounded-lg overflow-hidden border border-slate-700"> {{old('body' , $data->body )}} </textarea>
            </div>
            <input type="checkbox" name="published" {{$data->published || old('published') ? "checked" : ""}}>
